...
df = pd.read_csv(os.path.join(BASE_DIR, 'housing_price_dataset.csv'))
...
def distribution():
    ...
    filtered = df if neighborhood == 'all' else df[df['Neighborhood'] == neighborhood]

    bins = pd.cut(filtered['Price'], bins=10)
    dist = bins.value_counts().sort_index()
    price_dist = [
        {'range': f"${int(iv.left / 1000)}k–${int(iv.right / 1000)}k", 'count': int(ct)}
        for iv, ct in dist.items()
    ]
